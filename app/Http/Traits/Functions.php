...
trait Functions
{
    ...
    public function appendToGlobalWarning($sWarning)
    {
        if (isset($GLOBALS['sWarningGlobal'])) $GLOBALS['sWarningGlobal'] .= '<br>' . $sWarning;
        $GLOBALS['sWarningGlobal'] = $sWarning;
    }
    
    public function readAndClearGlobalWarning($isClear=true)
    {
    ...
    public function debug($sDebugMessage)
    {
    ...
    public function escpeHtmlQuill(&$string, $iMaxCharLength=5000)
    {
    ...
    public function escapeScriptTag($string)
    {
    ...
    public function todayDateForSql()
    {
        return date("Y-m-d");
    }
    
    public function currentMonthForSql()
    {
        return date("Y-m");
    }
    
    public function nowTimeForSql()
    {
        return date("Y-m-d H:i:s");
    }
    
    public function nowUtcTimeForSql()
    {
        if (defined('SERVER_LIVE'))
    ...
    public function nowOnlyTimeForSql()
    {
        return date("H:i:s");
    }
    
    public function timeForSqlFromNow($timeStringInWords)
    {
        return date("Y-m-d H:i:s", strtotime($timeStringInWords));
    }
    
    public function lastMonthForSql()
    {
    ...
    public function previousMonthStartDateString()
    {
        return date("Y-m", strtotime('-1 month')).'-01';
    }
    
    public function previousMonthLastDateString()
    {
        return date("Y-m-t", strtotime('-1 month'));
    }
    
    public function currentMonthStartDateString()
    {
        return date("Y-m").'-01';
    }
    
    public function currentMonthLastDateString()
    {
        return date("Y-m-t");
    }
    
    public function givenDateLastDateString($tdStartDateOfMonth)
    {
        return date("Y-m-t", strtotime($tdStartDateOfMonth));
    }
    
    public function datetimeFromBrowserDatetime($timeStringInWords)
    {
        // $date = DateTime::createFromFormat("Y-m-d\TH:i", $timeStringInWords);
        $date = date_create_from_format("Y-m-d\TH:i", $timeStringInWords);
        if ($date === false) return '';
        return $date->format('Y-m-d H:i:s');
    }
    
    public function datetimeToBrowserDatetime($datetime)
    {
        return date("Y-m-d\TH:i", strtotime($datetime));
    }
    
    public function getDateNDaysAfter($daysAfter)
    {
        return date("Y-m-d", strtotime("{$daysAfter} days"));
    }
    
    public function getDateNDaysBack($daysBack)
    {
        return date("Y-m-d", strtotime("-{$daysBack} days"));
    }
    
    public function getDatetimeNDaysBack($daysBack)
    {
        return date("Y-m-d H:i:s", strtotime("-{$daysBack} days"));
    }
    
    public function getDateNextWorkingDay()
    {
    ...
    public function getDateRecentWorkingDay()
    {
        return date("Y-m-d", strtotime("-18 hours"));
    }
    
    public function convertTimeZoneAddMinutes($time=null, $fromTimeZone='UTC', $toTimeZone='UTC', $iAfterMinutes=0)
    {
    ...
    public function utcTimestampToDefaultDatetime($datetime, $sDateFormat='d M Y g:i A T')
    {
    ...
    public function utcTimestampToDefaultDatetimeAfterMinutes($datetime, $minutes)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
        if (defined('SERVER_LIVE'))
        {
    ...
    public function unixTimestampToDatetime($timestamp)
    {
    ...
    public function defaultDatetimeToUtcTimestamp($datetime)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
    ...
    public function istTimeToGoogleRecordingTime($datetime)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
    ...
    public function timeToNicetime($time)
    {
    ...
    public function datetimeToFullDatetime($datetime)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
    ...
    public function datetimeToNiceDatetime($datetime, $convertAlways=false, $isUTC=false)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
    ...
    public function datetimeToNiceShortDatetime($datetime, $sPrefixIfNotNull=null, $sReturnIfNull='-')
    {
    ...
    public function datetimeToNiceShortTime($datetime, $isUTC=false, $sPrefixIfNotNull=null, $sReturnIfNull='-')
    {
        if (($datetime == null) || ($datetime == '')) return $sReturnIfNull;
        if(defined('SERVER_LIVE') && $isUTC)
        {
    ...
    public function datetimeToNiceShortDate($datetime, $sPrefixIfNotNull=null, $sReturnIfNull='-')
    {
        $datetime = trim($datetime);
        if (($datetime == null) || ($datetime == '')) return $sReturnIfNull;
        $sResult = $sReturnIfNull;
    ...
    /*
    public function datetimeToNiceShortDate($datetime)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
        //Table view has sorting which needs date in 2019-12-31 format
        if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'list') && ((!isset($_SESSION['listView'])) || ($_SESSION['listView'] == 'table')))
    ...
    public function datetimeToNiceShortDateWithDay($datetime)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
        $timestamp =  strtotime($datetime);
        if ($timestamp === false) return '-';
    ...
    public function dateTimeToDateWithDay($datetime)
    {
        if(($datetime == null) || ($datetime == '')) return '-';
        $timestamp =  strtotime($datetime);
        if ($timestamp === false) return '-';
    ...
    public function datetimeToNiceShortMonthDate($datetime)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
    ...
    public function datetimeToDmy($datetime)
    {
        if (($datetime == null) || ($datetime == '')) return '-';
    ...
    public function datetimeToIcalUtcTimeFormat($sDatetime=null, $sTimezone="Asia/Calcutta")
    {
        /*
        if ($sDatetime == null)
            $unixTime = time();
    ...
    public function timeToNearQuarterhour($timeOnly)
    {
    ...
    public function isDateTimeInRange($datetimeRef, $minutes, $datetime)
    {
    ...
    public function getDateTimeAfterMinutes($datetimeRef, $minutes)
    {
        if (empty($datetimeRef)) $datetimeRef = date('Y-m-d H:i:s');
        if (empty($minutes)) $minutes = 0;
        $DateTimeRef = new DateTime($datetimeRef);
    ...
    public function firstWord($statement, $isNameForEmail = true)
    {
        if ($isNameForEmail)
        {
    ...
    public function firstCharacter($sName)
    {
        return strtoupper(substr(firstWord($sName), 0, 1));
    }
    
    public function fullNameFromEmail($sEmailAddressOnly)
    {
        $sEmailExploded = explode('@', $sEmailAddressOnly);
        return ucfirst(trim(str_replace('.', ' ', $sEmailExploded[0])));
    }
    
    public function validateIdTokenAndExitIfFail($tableName)
    {
    ...
    public function validateAndGetLoginDetails($idToken, $sUserType)
    {
    ...
    public function closeConnectionWithOutputAndContinueProcessing($output)
    {
    ...
    public function sendSms($sMessage, $sMobileWithoutQuoates)
    {
        global $credentials;
    ...
    public function sendSmsOtp($otp, $sMobileWithoutQuoates)
    {
        global $credentials;
    ...
    public function sendSmsChooseLink($sTokenInterest, $id, $sMobileWithoutQuoates, $sName)
    {
        global $credentials;
    ...
    */
    ...
    public function validateSessionParams()
    {
    ...
    public function forceReloginOtherUser($sEmail)
    {
    ...
    public function checkRole($bitmap)
    {
    ...
    public function isUserTypeEmployee()
    {
    ...
    public function resetSessionWithKeepForward($isForwardAfterLogin = true)
    {
        sessionStart();
        $fwd = null;
    ...
    public function logout($isForwardAfterLogin = true)
    {
    ...
    public function isBitSetInBitmap($iBitmap, $iIndex, $isReturnInStringFormat=false)
    {
    ...
    public function bitsToBitmap($arrayBits)
    {
        $bitmap = 0;
    ...
    public function multiSelectToString($arrOptions)
    {
        $select = '';
    ...
    //Convert Select multiple select options to string with comma
    public function multiOptionsToSelectString($arrOptions)
    {
        $select = '';
        if (gettype($arrOptions) == 'array')
        {
    ...
    public function multiSelectToMdd($tableName, $existingMdd, $arrSelect)
    {
    ...
    public function mddToObjectNames($columnName, $mdd)
    {
        if (empty($mdd)) return '';
        $mdd = makeSafe($mdd);
    ...
    public function mddToObjectNamesIvr($mdd, $id_jd=null)
    {
        if (empty($mdd)) return '';
        $mdd = makeSafe($mdd);
        $sObjectNames = '';
    ...
    public function isIdPresentInMdd($id, $sMddCommaSeperated)
    {
    ...
    public function jdNameWithoutCorp($statement)
    {
        if (empty($statement)) return '';
        else
        {
    ...
    public function wordsAfterDash($statement)
    {
        if (empty($statement)) return '';
        else return explode('-',trim($statement))[1];
    }
    
    public function strStartsWith($string, $subString)
    {
        $len = strlen($subString);
        return (substr($string, 0, $len) === $subString);
    }
    
    public function strEndsWith($string, $subString)
    {
        $len = strlen($subString);
        if ( !$len ) {
            return true;
    ...
    public function paddingByRandomString($key,$lengthPadding)
    {
        global $base32Chars;
    ...
    public function hackingAttempt($sOtherMessage = null, $isExit = true)
    {
    ...
    //This is called when invalid tokens and Ids are provided by user
    public function hackingAttemptByAPI($sLogMessage = null, $isExit = true)
    {
    ...
    /*
    public function getCallingLink($sMobile)
    {
        $len = strlen($sMobile);
    ...
    */
    /*
    public function getCallingLink($sMobile, $isHref=true)
    {
        $sMobileCallingLinks = "";
    ...
    */
    ...
    public function getCallingLink($sMobile, $isHref=true, $isForWhatsApp=false)
    {
        $sMobileCallingLinks = "";
        if (strStartsWith($sMobile, '[{'))
        {
    ...
    public function isMobileHasIsd($sMobile)
    {
        return strStartsWith($sMobile, "+");
    }
    /*
    public function getMobileWithIsd($sMobile)
    {
        $len = strlen($sMobile);
        if ($len < 10) return $sMobile;
        else if ($len == 10) $prefix = "+91";
    ...
    */
    public function getMobileWithIsd($sMobile, $sIsdSelected='+91') //default india
    {
    ...
    public function extractOnlyMobileWithoutIsd($sMobile)//ct
    {
    ...
    public function createEvent($tableName, $id, $xdEventType, $sParam0, $sParam1, $sParam2, $sParam3, $sParam4, $sParam5)
    {
    ...
    public function createActiveLog($enum_table_hash, $id, $xdActivityType, $iaSubType=null, $iaResult=null, $iaTableRef=null, $iaItemRef=null)
    {
    ...
    public function isValidRequest($parameter)
    {
    ...
    public function getFileAbsoutePath($sFilePath)
    {
    ...
    //Using composer.json
    public function loadComposerPsr4Package($dir)
    {
        ...
        foreach ($namespaces as $namespace => $classpaths) {
            ...
            // spl_autoload_register(public function ($classname) use ($namespace, $classpaths, $dir) {
    ...
    public function getCandExpMinForPanel($iExpPanel, $iIvrBonus=500)
    {
    /*
    ...
    */
    ...
    public function getCandExpMaxForPanel($iExpPanel, $iIvrBonus=500)
    {
    ...
    public function getMaxAndCriticalBonusForIv($id)
    {
    ...
    public function splitAndCleanNameEmailAddress($sNameEmailWithBracket)
    {
        $sName = null;
    ...
    public function emailsStringToArray($to)
    {
    ...
    public function emailsStringToEmailOnlyArray($to)
    {
        $arrEmail = array();
        //Replace all semicolon with comma
        $to = str_replace(';', ',', $to);
    ...
    public function getRootRelatedPathFromControlRelatedPath($controlRelatedPath)
    {
    ...
    //If standing at current month 2022-03 then public function return ['2022-03', '2022-02', '2022-01','2021-12', '2021-11',........'2021-01', '2020-12', .........]
    public function getPreviousMonthsNamesArray($noOfMonths)
    {
        //Create blank array
        $arrYearMonths = array();
        //Current year
    ...
    //public functions to get first and last dates of Financial year based on invoice date.
    public function startDateOfFinancialYear($tdInvoiceDate = null)
    {
    ...
    public function startYearOfFinancialYear($tdInvoiceDate = null)
    {
        //1 april-31 dec same year
        //1 jan-31 march- year-1
        if ($tdInvoiceDate == null) $tdInvoiceDate = date("Y-m-d");
    ...
    public function startYearOfPrevFinancialYear($tdInvoiceDate = null)
    {
        //1 april-31 dec same year
        //1 jan-31 march- year-1
        if ($tdInvoiceDate == null) $tdInvoiceDate = date("Y-m-d");
    ...
    public function lastDateOfFinancialYear($tdInvoiceDate = null)
    {
        if ($tdInvoiceDate == null) $tdInvoiceDate = date("Y-m-d");
        $time = strtotime($tdInvoiceDate);
        $iInvoiceYear = intval(date("Y", $time));
    ...
    public function lastYearOfFinancialYear($tdInvoiceDate = null)
    {
        if ($tdInvoiceDate == null) $tdInvoiceDate = date("Y-m-d");
        $time = strtotime($tdInvoiceDate);
        $iInvoiceYear = intval(date("Y", $time));
    ...
    public function lastYearOfPrevFinancialYear($tdInvoiceDate = null)
    {
        if ($tdInvoiceDate == null) $tdInvoiceDate = date("Y-m-d");
        $time = strtotime($tdInvoiceDate);
        $iInvoiceYear = intval(date("Y", $time));
    ...
    public function getAmountNumberToWords(float $number)
    {
    ...
    public function getMobileFromJsonFormat($sMobile)
    {
        $sMobileWithComma = "";
        if (strStartsWith($sMobile, '[{'))
        {
    ...
    public function decodeJsonAndCheckError($string, &$sError, $isErrorLog=true)
    {
    ...
    public function getMonthNumberFromDate($tdDate)
    {
    ...
    public function getHoursMinutesStringFromDecimalHours($fHours)
    {
    ...
    public function getRupeesPaisefromDecimalRupees($fRupees)
    {
        $iRupeesPaise = "";
    ...
    public function getCurrentMonthOneToTwelve()
    {
    ...
    public function isSubstringFoundCaseInsensitive($string, $subString)
    {
    ...
    public function saveFormOpenTimeInSession()
    {
        $_SESSION['tdtAddEditFormOpenTime'] = nowTimeForSql(); //"2023-06-02 11:40:00"; //
    }
    
    public function getDurationFormOpenTimeInSession()
    {
    ...
    public function secondsRemainingForTimeFromNow($datetime)
    {
    ...
    public function sessionEditFormOpenTimeSet()
    {
        $_SESSION['EditFormOpenTime'] = nowTimeForSql();
    }
    
    public function sessionEditFormOpenTimeReadAndClear()
    {
        $_SESSION['EditFormOpenTime'] = nowTimeForSql();
    }
    ...
    public function getGoogleDriveFileIdFromUrl($sFileUrl)
    {
    ...
    public function getTimeInHrs($iMinutes)
    {
    ...
    public function arrayKeyStartsWithId($array)
    {
    ...
    public function isValidEmail($sEmail, $isToBeReplaced=false)
    {
    ...
    public function validateEmailFormatOrExit($sEmail, $sErrorMsg=null, $isExitOnError=true, $isToBeReplaced=false, $isAllowMultipleEmails=true)
    {
    ...
    public function generateApiResponseJson($isSuccess, $sResponseError, $objData)
    {
    ...
    public function convertHtmlToText($sHtml)
    {
    ...
    public function getDeviceAndOsType()
    {
    ...
    public function isEditLoginEmailPassword($xcProfile)
    {
        global $sUserType;
    ...
    public function sendNotification($arrTo, $sTitle, $sMessagae, $sLinkClickAction)
    {
        // $to is device token
    ...
    public function verifyGoogleCaptcha()
    {
    ...
    public function getViewGoogleCaptcha()
    {
        $output = "
    ...
    public function makeSafe($sString)
    {
        return $sString;
    }
    public function makeSafeAndSingleQuote($sString)
    {
        $a = "'" . $sString . "'";
        return $a;
    }
    public function webErrorMessage($sString1, $sString2)
    {
        echo $sString1;
    }
